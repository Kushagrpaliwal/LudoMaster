...
const LudoBoard = () => {
  ...
  useEffect(() => {
    ...
    if (sessionId) {
      // There's an active session - restore the game
      console.log("Found active session, restoring game...");
      restoreGameSession(sessionId);
    } else {
      // No active session - auto-start a new free play game
      console.log("Starting new free play game...");
  ...
  const [currentPlayer, setCurrentPlayer] = useState(0); // 0 = Red (Human), 1 = Yellow (Bot)
  const [showPaymentModal, setShowPaymentModal] = useState(false);
  const [consecutiveSixes, setConsecutiveSixes] = useState({ 0: 0, 1: 0 });
  ...
  const [gameMessage, setGameMessage] = useState("Pay 100 to start playing!");
  const [botThinking, setBotThinking] = useState(false);
  const [userWallet, setUserWallet] = useState(1000);
  const [gameProcessed, setGameProcessed] = useState(false);
  const [winningScreenShown, setWinningScreenShown] = useState(false); // Track if winning screen has been shown
  const [losingScreenShown, setLosingScreenShown] = useState(false); // Track if losing screen has been shown
  ...
  const [botFirstTurnAfterReload, setBotFirstTurnAfterReload] = useState(false); // Track bot's first turn after reload
  ...
  const startTimer = (playerIndex) => {
    ...
    const interval = setInterval(() => {
      ...
      setTimeLeft((prev) => {
        ...
        if (prev <= 5 && prev > 0 && !audioPlayedRef.current) {
          if (timerAudioRef.current) {
            console.log("Playing timer audio for last 5 seconds:", prev);
        ...
        // Reset audio flag for next second only when timer is above 5 seconds
        if (prev > 5) {
        ...
        if (prev <= 1 && !timerExpired && !timeoutProcessedRef.current) {
          ...
          setRedDots((prevDots) => {
            ...
            // Check if player gets 3 red dots and loses
            if (newRedDots[playerIndex] >= 3) {
              // Stop timer immediately when game ends due to timeouts
              stopTimer();
              ...
              if (playerIndex === 1) {
                ...
                setGameMessage("Bot lost! Too many timeouts!");
          ...
          // When timer expires, just pass turn to user (no auto-move)
          if (playerIndex === 1) {
            // Bot's turn expired - pass to user
            setCurrentPlayer(0);
            setBotNeedsToRoll(false);
            setGameMessage("Time's up! Your turn.");
          } else if (playerIndex === 0) {
            // User's turn expired - add red dot and pass to bot
            setCurrentPlayer(1);
            setBotNeedsToRoll((prev) => !prev);
            setGameMessage("Time's up! Bot's turn.");
  ...
  const stopTimer = () => {
    ...
    // Stop timer audio
    if (timerAudioRef.current) {
      console.log("Stopping timer audio - timer stopped");
      timerAudioRef.current.pause();
      timerAudioRef.current.currentTime = 0;
    }
  ...
  const fetchWalletBalance = async () => {
    try {
      ...
      if (!phone) {
        console.log("No phone found, cannot fetch wallet balance");
        return;
      }
      ...
      if (response.ok) {
        ...
        if (response.status === 404) {
          window.location.href =
            "/profile/login?redirect=/games/ludo/TwoPlayerLudo";
        }
      }
    } catch (error) {
  ...
  const [redDots, setRedDots] = useState({ 0: 0, 1: 0 }); // Track red dots for each player
  ...
  useEffect(() => {
    const fetchGameRooms = async () => {
      try {
        const response = await fetch('/api/gamerooms?playerCount=2');
        const data = await response.json();
        if (data.success) {
  ...
  const playerStartPositions = {
    0: 26, // Red (user) now starts at position 26 (yellow's old position)
    1: 0, // Yellow (bot) now starts at position 0 (red's old position)
  };
  ...
  const restoreGameSession = async (sessionId) => {
    try {
      console.log("Restoring game session:", sessionId);

      // Check if session has expired (60 minutes)
      const sessionStartTime = localStorage.getItem("ludoSessionStartTime");
      if (sessionStartTime) {
        ...
        if (sessionAge > maxSessionAge) {
          console.log("2-player session has expired, cleaning up...");
  ...
  const restoreFromLocalStorage = () => {
    try {
      ...
      const savedRoom = localStorage.getItem("ludoSelectedRoom");
      const sessionStartTime = localStorage.getItem("ludoSessionStartTime");
      ...
      if (savedGameState && savedRoom) {
        ...
        // Restore comprehensive turn state from localStorage
        const savedCurrentPlayer = localStorage.getItem("ludoCurrentPlayer");
        const savedDiceValue = localStorage.getItem("ludoDiceValue");
        const savedConsecutiveSixes = localStorage.getItem(
          "ludoConsecutiveSixes"
        );
        const savedRedDots = localStorage.getItem("ludoRedDots");
        const savedHasRolledThisTurn = localStorage.getItem(
          "ludoHasRolledThisTurn"
        );
        const savedBotNeedsToRoll = localStorage.getItem("ludoBotNeedsToRoll");
        const savedBotThinking = localStorage.getItem("ludoBotThinking");
        const savedBotFirstTurnAfterReload = localStorage.getItem(
          "ludoBotFirstTurnAfterReload"
        );
        ...
        if (savedHasRolledThisTurn)
          setHasRolledThisTurn(savedHasRolledThisTurn === "true");
        if (savedBotNeedsToRoll)
          setBotNeedsToRoll(savedBotNeedsToRoll === "true");
        if (savedBotThinking) setBotThinking(savedBotThinking === "true");
        if (savedBotFirstTurnAfterReload)
          setBotFirstTurnAfterReload(savedBotFirstTurnAfterReload === "true");

        console.log("Restored turn state from localStorage:", {
          currentPlayer: savedCurrentPlayer
            ? parseInt(savedCurrentPlayer)
            : "not found",
          hasRolledThisTurn: savedHasRolledThisTurn,
          botNeedsToRoll: savedBotNeedsToRoll,
          botThinking: savedBotThinking,
          botFirstTurnAfterReload: savedBotFirstTurnAfterReload,
        });
        ...
        console.log("No saved game state found, showing game rooms");
  ...
  const handleBackToLobby = () => {
    ...
    localStorage.removeItem("currentLudoSessionId");
    localStorage.removeItem("ludoGameState");
    localStorage.removeItem("ludoSelectedRoom");
    localStorage.removeItem("ludoCurrentPlayer");
    localStorage.removeItem("ludoDiceValue");
    localStorage.removeItem("ludoConsecutiveSixes");
    localStorage.removeItem("ludoRedDots");
    localStorage.removeItem("ludoHasRolledThisTurn");
    localStorage.removeItem("ludoBotNeedsToRoll");
    localStorage.removeItem("ludoBotThinking");
    localStorage.removeItem("ludoBotFirstTurnAfterReload");
    localStorage.removeItem("ludoSessionStartTime");
    localStorage.removeItem("ludoLastSaveTime");
    localStorage.removeItem("ludoLastActionTime");
  ...
  const handleExitGame = () => {
    // const isConfirmed = window.confirm("⚠️ Are you sure you want to exit the game? Your current progress will be lost and the session will be ended.");
  ...
  // Function to check for suspicious reload patterns
  const checkForSuspiciousReload = (
    currentPlayer,
    hasRolledThisTurn,
    userHasMovedTokens
  ) => {
    const lastActionTime = localStorage.getItem("ludoLastActionTime");
    const currentTime = Date.now();

    if (lastActionTime) {
      const timeSinceLastAction = currentTime - parseInt(lastActionTime);

      // If user just took action and reloaded within 10 seconds, it's suspicious
      if (
        currentPlayer === 0 &&
        hasRolledThisTurn &&
        userHasMovedTokens &&
        ...
        console.warn(
          "SUSPICIOUS RELOAD DETECTED: User completed turn and reloaded within 10 seconds"
        );
        return true;
      }
    }
  ...
  // Function to check if this is a reload and restore accurate turn state
  const checkForReloadAndRestoreTurn = () => {
    const lastSaveTime = localStorage.getItem("ludoLastSaveTime");
    const sessionStartTime = localStorage.getItem("ludoSessionStartTime");
    const currentTime = Date.now();

    if (lastSaveTime && sessionStartTime) {
      ...
      // If game was saved recently and session is active, this might be a reload
      if (timeSinceLastSave < 60000 && sessionAge < 60 * 60 * 1000) {
        // 60 seconds
        console.log(
          "Reload detected - restoring accurate turn state from localStorage..."
        );

        // Get all turn-related information from localStorage
        const savedCurrentPlayer = localStorage.getItem("ludoCurrentPlayer");
        const savedHasRolledThisTurn = localStorage.getItem(
          "ludoHasRolledThisTurn"
        );
        ...
        if (savedCurrentPlayer && savedGameState) {
          const gameState = JSON.parse(savedGameState);
          const currentPlayer = parseInt(savedCurrentPlayer);
          const hasRolledThisTurn = savedHasRolledThisTurn === "true";
          const botNeedsToRoll = savedBotNeedsToRoll === "true";
          const botThinking = savedBotThinking === "true";
          const botFirstTurnAfterReload =
            savedBotFirstTurnAfterReload === "true";

          // Check if user has already completed their turn (rolled and moved)
          const userTokens = gameState.players[0].tokens;
          const userHasMovedTokens = userTokens.some(
            (token) =>
              !token.isHome || token.isInHomeStretch || token.isFinished
          );
          const userHasCompletedTurn = hasRolledThisTurn && userHasMovedTokens;

          console.log("Restoring accurate turn state from localStorage:", {
            currentPlayer,
            hasRolledThisTurn,
            botNeedsToRoll,
            botThinking,
            botFirstTurnAfterReload,
            userHasMovedTokens,
            userHasCompletedTurn,
            gameStarted: gameState.gameStarted,
          });

          // Restore the exact turn state from localStorage
          setCurrentPlayer(currentPlayer);
          setHasRolledThisTurn(hasRolledThisTurn);
          setBotNeedsToRoll(botNeedsToRoll);
          setBotThinking(botThinking);
          setBotFirstTurnAfterReload(botFirstTurnAfterReload);

          if (currentPlayer === 1) {
            // Bot's turn - restore exactly as it was
            setGameMessage("Bot's turn (restored from localStorage)");

            // If bot was supposed to roll, trigger it
            if (botNeedsToRoll && !botThinking) {
              setTimeout(() => {
                console.log("Triggering bot turn as per localStorage state...");
                setBotNeedsToRoll((prev) => !prev);
              }, 1000);
            }
          } else {
            // User's turn - check for suspicious reload first
            const isSuspiciousReload = checkForSuspiciousReload(
              currentPlayer,
              hasRolledThisTurn,
              userHasMovedTokens
            );

            if (isSuspiciousReload) {
              // Force switch to bot for suspicious reload
              console.log("Suspicious reload detected - forcing bot turn");
              setCurrentPlayer(1);
              setGameMessage("Bot's turn (suspicious reload detected)");
              setBotNeedsToRoll(true);
              setHasRolledThisTurn(false);
            } else if (userHasCompletedTurn) {
              // User has already rolled and moved tokens - switch to bot to prevent double turn
              console.log(
                "User already completed their turn (rolled and moved) - switching to bot to prevent double turn"
              );
              setCurrentPlayer(1);
              setGameMessage("Bot's turn (user already completed their turn)");
              setBotNeedsToRoll(true);
              setHasRolledThisTurn(false);
            } else if (hasRolledThisTurn && !userHasMovedTokens) {
              // User rolled but hasn't moved tokens yet - let them continue
              console.log(
                "User rolled but hasn't moved tokens yet - continuing their turn"
              );
              setGameMessage("Your turn - select a token to move");
              startTimer(0);
            } else {
              // User hasn't rolled yet - continue their turn
              console.log("User hasn't rolled yet - continuing their turn");
              setGameMessage("Your turn (restored from localStorage)");
              startTimer(0);
            }
          }
          ...
          // No localStorage data - this is a fresh game, not a reload
          console.log(
            "No localStorage turn data found - this appears to be a fresh game"
          );
          return false;
        }
      }
  ...
  const handleJoinGame = async (roomOverride = null) => {
    console.log("handleJoinGame called - Starting free play game!");
    ...
    console.log("Starting game with room:", roomToJoin);
    ...
    localStorage.setItem("currentLudoSessionId", sessionId);

    // Reset timer and red dots for new game
    setRedDots({ 0: 0, 1: 0 });
    stopTimer();

    // For fresh game, ensure user gets first turn and game starts
    console.log("Fresh game - User gets first turn");
    setTimeout(() => {
    ...
    localStorage.setItem("ludoGameState", JSON.stringify(initialGameState));
    localStorage.setItem("ludoSelectedRoom", JSON.stringify(roomToJoin));
    localStorage.setItem("ludoCurrentPlayer", "0");
    localStorage.setItem("ludoDiceValue", "6");
    localStorage.setItem("ludoConsecutiveSixes", JSON.stringify({ 0: 0, 1: 0 }));
    localStorage.setItem("ludoRedDots", JSON.stringify({ 0: 0, 1: 0 }));
    localStorage.setItem("ludoHasRolledThisTurn", "false");
    localStorage.setItem("ludoBotNeedsToRoll", "false");
    localStorage.setItem("ludoBotThinking", "false");
    localStorage.setItem("ludoBotFirstTurnAfterReload", "false");
    localStorage.setItem("ludoSessionStartTime", Date.now().toString());
    localStorage.setItem("ludoLastSaveTime", Date.now().toString());
    localStorage.setItem("ludoLastActionTime", Date.now().toString());
  ...
  useEffect(() => {
    ...
    // Stop background music and timer if there's a winner
    if (gameState.winner !== null) {
      console.log("Game has a winner, stopping background music and timer");
  ...
  useEffect(() => {
    const sessionCheckInterval = setInterval(() => {
      ...
      if (sessionId && sessionStartTime) {
        ...
        if (sessionAge > maxSessionAge) {
          console.log(
            "Periodic check: 2-player session has expired, cleaning up..."
          );
          cleanupExpiredSession();
        }
      }
  ...
  useEffect(() => {
    ...
    // Stop background music if there's a winner
    if (gameState.winner !== null && audio) {
      console.log("Game has a winner, stopping background music");
      audio.pause();
      audio.currentTime = 0;
      return;
    }
  ...
  useEffect(() => {
    ...
    // Stop all game logic if there's a winner
    if (gameState.winner !== null) {
      console.log("Game has a winner, stopping bot logic");
      return;
    }

    // Allow bot to take its first turn after reload, but prevent additional turns
    // The botFirstTurnAfterReload flag will be reset after the bot completes its move

    // Debug: Log why bot turn might not be starting
    if (
      currentPlayer === 1 &&
      !botThinking &&
      !isRolling &&
      ...
      console.log("Bot turn conditions met, should start bot turn");
    } else if (currentPlayer === 1) {
      console.log("Bot turn blocked because:", {
        botThinking,
        isRolling,
        isMovingToken,
        gameStarted: gameState.gameStarted,
    ...
    } else if (currentPlayer === 1 && !gameState.gameStarted) {
      console.log("Bot turn but game not started yet, starting game...");
      setGameState((prev) => ({ ...prev, gameStarted: true }));
      setBotThinking(true);
      setGameMessage("Bot is thinking...");
      setTimeout(() => handleBotTurn(), 1500);
    ...
    isMovingToken,
    isRolling,
    botThinking, // Add missing dependency
  ]);

  // Timer logic for user turn (matches FourPlayerLudo)
  useEffect(() => {
    if (
      currentPlayer === 0 &&
      gameState.gameStarted &&
      ...
      console.log("Starting timer for user's turn (auto-start)");
      startTimer(0);
  ...
  const wouldCapture = (playerIndex, tokenIndex, diceValue) => {
    ...
    if (newStepsFromStart < 51) {
      ...
      // Additional check for star positions and starting positions
      const safePositions = [
        0, // Red start position
        8, // First star position (safe square)
        13, // Green start position (for 2-player, this is yellow's path)
        22, // Second star position (safe square)
        26, // Yellow start position
      ...
      // Additional check: If the position has a star visual, it's also safe
      // These are the visual star positions that might not be in boardPath
      const visualStarPositions = [0, 8, 26, 39, 47, 13, 21, 34]; // Include home area star positions (6, 14) and main board stars (21, 34)

      if (visualStarPositions.includes(newPos)) {
        return false;
      }
      ...
      return gameState.players[otherPlayerIndex].tokens.some(
        (oppToken) =>
          !oppToken.isHome &&
          !oppToken.isFinished &&
  ...
  const hasAnyValidMoves = (playerIndex) => {
    ...
    const allTokensAtHome = gameState.players[playerIndex].tokens.every(
      (token) => token.isHome
    );

    // If all tokens are at home, only 6 allows movement
    if (allTokensAtHome) {
      const validMoves = getValidMoves(playerIndex, 6);
      const result = validMoves.length > 0;
      console.log(`hasAnyValidMoves for player ${playerIndex} (all at home):`, {
        allTokensAtHome,
        validMoves,
        result,
      });
  ...
  const moveToken = async (playerIndex, tokenIndex, diceValue) => {
    // Stop token movement if there's a winner
    if (gameState.winner !== null) {
      console.log("Game has a winner, token movement blocked");
      return;
    }

    console.log("moveToken called:", {
      playerIndex,
      tokenIndex,
      diceValue,
      currentPlayer,
    ...
    // CRITICAL CHECK: Ensure only the correct player's tokens can move
    if (playerIndex !== currentPlayer) {
      console.error(
        "INVALID MOVE: Player",
        playerIndex,
        "trying to move when currentPlayer is",
        currentPlayer
      );
    ...
    // Additional check: Ensure token belongs to the correct player
    if (playerIndex === 0 && tokenIndex >= 4) {
      console.error("INVALID TOKEN: User trying to move bot token", tokenIndex);
      setIsMovingToken(false);
      return;
    }
    if (playerIndex === 1 && tokenIndex >= 4) {
      console.error("INVALID TOKEN: Bot trying to move user token", tokenIndex);
      setIsMovingToken(false);
      return;
    }
    ...
    if (diceValue === 6 || tokenCaptured) {
      if (diceValue === 6) {
        ...
        if (newPlayerSixes >= 3) {
          ...
          // Reset turn states for the next player
          if (nextPlayer === 0) {
            // User's turn - reset hasRolledThisTurn so they can roll
            setHasRolledThisTurn(false);
            setBotNeedsToRoll(false);
            setGameMessage("Your turn - 3 sixes cancelled previous turn!");
            // Start timer for user's turn
            startTimer(0);
          } else {
            // Bot's turn - trigger bot turn
            setHasRolledThisTurn(false);
            setBotNeedsToRoll(true);
            setGameMessage("Bot's turn - 3 sixes cancelled previous turn!");
          }
        } else {
          setGameMessage(
          ...
          if (playerIndex === 1) {
            console.log("Bot got a 6, triggering another roll...");
            setBotNeedsToRoll((prev) => !prev); // Toggle to trigger useEffect
            // Ensure bot's turn continues
            setCurrentPlayer(1);
          } else {
        ...
        // If it's the bot's turn, trigger another roll
        if (playerIndex === 1) {
          console.log("Bot captured a token, triggering another roll...");
          setBotNeedsToRoll((prev) => !prev); // Toggle to trigger useEffect
          // Ensure bot's turn continues
          setCurrentPlayer(1);
        } else {
      ...
      if (tokenFinished) {
        ...
        if (playerIndex === 0) {
          ...
          console.log("Bot finished a token, triggering another roll...");
          setBotNeedsToRoll((prev) => !prev);
          // Ensure bot's turn continues
          setCurrentPlayer(1);
        }
        ...
        console.log("Normal turn switching:", {
          playerIndex,
          botFirstTurnAfterReload,
          currentPlayer,
        });
        ...
        const nextPlayer = playerIndex === 0 ? 1 : 0;
        console.log("Switching turn from", playerIndex, "to", nextPlayer);
        setCurrentPlayer(nextPlayer);
  ...
  const startSessionTimeout = () => {
    ...
    console.log("Starting 60-minute session timeout for 2-player game");
    setSessionStartTime(Date.now());

    // Set timeout for 60 minutes (60 * 60 * 1000 = 3,600,000 ms)
    sessionTimeoutRef.current = setTimeout(() => {
      console.log(
        "2-player session timeout reached (60 minutes), cleaning up..."
      );
      cleanupExpiredSession();
    }, 60 * 60 * 1000);
  };
  ...
  const cleanupExpiredSession = async () => {
    try {
      ...
      console.log("Cleaning up expired 2-player session:", sessionId);
      ...
      setCurrentPlayer(0);
      setDiceValue(6);
      setConsecutiveSixes({ 0: 0, 1: 0 });
      setRedDots({ 0: 0, 1: 0 });
      setGameMessage("Session expired. Please start a new game.");

      console.log("2-player session cleanup completed");
    } catch (error) {
  ...
  const stopSessionTimeout = () => {
    ...
    console.log("2-player session timeout stopped");
  };
  ...
  const updateSessionState = async (gameState) => {
    try {
      ...
      // Save comprehensive turn information to localStorage
      localStorage.setItem("ludoGameState", JSON.stringify(gameState));
      localStorage.setItem("ludoSelectedRoom", JSON.stringify(selectedRoom));
      localStorage.setItem("ludoCurrentPlayer", currentPlayer.toString());
      localStorage.setItem("ludoDiceValue", diceValue.toString());
      localStorage.setItem(
        "ludoConsecutiveSixes",
        JSON.stringify(consecutiveSixes)
      );
      localStorage.setItem("ludoRedDots", JSON.stringify(redDots));
      localStorage.setItem(
        "ludoHasRolledThisTurn",
        hasRolledThisTurn.toString()
      );
      localStorage.setItem("ludoBotNeedsToRoll", botNeedsToRoll.toString());
      localStorage.setItem("ludoBotThinking", botThinking.toString());
      localStorage.setItem(
        "ludoBotFirstTurnAfterReload",
        botFirstTurnAfterReload.toString()
      );
      localStorage.setItem("ludoLastSaveTime", Date.now().toString());
      localStorage.setItem("ludoLastActionTime", Date.now().toString());

      console.log("Updated localStorage with turn info:", {
        currentPlayer,
        hasRolledThisTurn,
        botNeedsToRoll,
        botThinking,
  ...
  const animateHomeStretchMove = async (
    ...
    for (let step = 1; step <= diceValue; step++) {
      ...
      if (currentPos <= 5) {
        ...
        // Note: Home stretch doesn't have visual star images, so no star sound here
        ...
        if (tokenHopAudioRef.current) {
          ...
          console.warn("tokenHopAudioRef.current is null (home stretch)");
        }
  ...
  const playStarSound = () => {
    if (starSoundRef.current) {
      console.log("Playing star sound");
      ...
      console.warn("starSoundRef.current is null - cannot play star sound");
    }
  };
  ...
  const animateMainBoardMove = async (
    ...
    for (let step = 1; step <= diceValue; step++) {
      ...
      if (newStepsFromStart >= 51) {
        ...
        // Star sound is now handled at the final position only, not during intermediate steps
    ...
    // Check if token landed on visual star images and play star sound
    // Only play star sound when token stops at visual star images, not on all star positions
    // Based on board layout analysis:
    // - Red star (right horizontal): position 21 (x: 12, y: 6)
    // - Blue star (bottom vertical): position 34 (x: 8, y: 11)
    // - Green home star: position 6 (x: 6, y: 2) - top vertical path
    // - Yellow home star: position 14 (x: 8, y: 3) - left horizontal path
    const visualStarPositions = [0, 8, 26, 39, 47, 13, 21, 34]; // Include home area star positions

    if (visualStarPositions.includes(finalPosition)) {
      console.log(
        `Playing star sound for visual star at position ${finalPosition}`
      );
      playStarSound();
    }
  ...
  const checkForCaptures = (
    ...
    // Define additional safe positions (star positions and starting positions)
    const safePositions = [
      0, // Red start position
      8, // First star position (safe square)
      13, // Green start position (for 2-player, this is yellow's path)
    ...
    // Check if the new position is a safe position
    ...
    if (visualStarPositions.includes(newPosition)) {
      console.log(
        `Position ${newPosition} is a visual star position, no capture possible`
      );
      return false;
    }
    ...
    // Count tokens of the other player at this position
    gameState.players[otherPlayerIndex].tokens.forEach((otherToken, index) => {
      if (
        !otherToken.isHome &&
  ...
  const handleBotTurn = async () => {
    console.log("handleBotTurn called - starting bot turn");

    // Stop bot turn if there's a winner
    if (gameState.winner !== null) {
      console.log("Game has a winner, bot turn cancelled");
    ...
    setIsRolling(true);
    setGameMessage("🤖 Bot is rolling dice...");
    ...
    setTimeout(async () => {
      ...
      setTimeout(async () => {
        const validMoves = getValidMoves(1, botDiceValue);

        // Special case: If all tokens are at home and bot rolled 6, they can move a token out
        const allTokensAtHome = gameState.players[1].tokens.every(
          (token) => token.isHome
        );
        const canMoveOutWithSix = allTokensAtHome && botDiceValue === 6;

        if (validMoves.length > 0 || canMoveOutWithSix) {
          // If all tokens are at home and we got a 6, find tokens that can move out
          let movesToConsider = validMoves;
          if (canMoveOutWithSix) {
            movesToConsider = gameState.players[1].tokens
          ...
          if (movesToConsider.length === 1) {
            setGameMessage("Bot is moving their only available token...");
            await moveToken(1, movesToConsider[0], botDiceValue);
          } else {
            // Multiple moves available - use AI strategy
            let chosenMove = movesToConsider[0];

            // Priority 1: Finish a token if possible
            const finishingMoves = movesToConsider.filter((tokenIndex) => {
              const token = gameState.players[1].tokens[tokenIndex];
              if (token.isInHomeStretch) {
            ...
            if (finishingMoves.length > 0) {
              ...
              const homeTokens = movesToConsider.filter(
                (tokenIndex) => gameState.players[1].tokens[tokenIndex].isHome
              );
              ...
              if (homeTokens.length > 0 && botDiceValue === 6) {
                ...
                const captureMoves = movesToConsider.filter((tokenIndex) => {
                  return wouldCapture(1, tokenIndex, botDiceValue);
                });
                ...
                if (captureMoves.length > 0) {
                  ...
                  chosenMove = movesToConsider.reduce((best, current) => {
                    const bestToken = gameState.players[1].tokens[best];
                    const currentToken = gameState.players[1].tokens[current];
                    ...
                    const playerStartPos = playerStartPositions[1];
            ...
            console.log("Bot moving token:", {
              playerIndex: 1,
              tokenIndex: chosenMove,
              diceValue: botDiceValue,
            });
            await moveToken(1, chosenMove, botDiceValue);
          }
        } else {
          // No valid moves
          console.log("Bot has no valid moves, switching to user turn");
          setConsecutiveSixes((prev) => ({ ...prev, 1: 0 }));
          setCurrentPlayer(0);
          setGameMessage("Bot has no valid moves. Your turn!");
          setBotNeedsToRoll(false); // Reset bot roll trigger
          setHasRolledThisTurn(false); // Reset for new turn
        }

        console.log("Bot turn completed, setting botThinking to false");
        setBotThinking(false);
      }, 1000);
    }, 1000);
  };
  ...
  const rollDice = () => {
    // Stop dice rolling if there's a winner
    if (gameState.winner !== null) {
      console.log("Game has a winner, dice rolling blocked");
      return;
    }
    ...
    // Stop timer once the user initiates the dice roll
    stopTimer();
    ...
    setTimeout(() => {
      ...
      // Special case: If all tokens are at home and player rolled 6, they can move a token out
      const allTokensAtHome = gameState.players[0].tokens.every(
        (token) => token.isHome
      );
      const canMoveOutWithSix = allTokensAtHome && newDiceValue === 6;

      console.log("User dice roll result:", {
        newDiceValue,
        validMoves,
        allTokensAtHome,
        canMoveOutWithSix,
        hasValidMoves: validMoves.length > 0 || canMoveOutWithSix,
      });

      if (validMoves.length > 0 || canMoveOutWithSix) {
        // If all tokens are at home and we got a 6, find tokens that can move out
        let movesToShow = validMoves;
        if (canMoveOutWithSix) {
          movesToShow = gameState.players[0].tokens
            .map((token, index) => ({ token, index }))
            .filter(({ token }) => token.isHome)
            .map(({ index }) => index);
        }

        setPossibleMoves(movesToShow);

        // Start timer for user's turn
        startTimer(0);

        // Check if any move can capture
        const captureMoves = movesToShow.filter((move) =>
          wouldCapture(0, move, newDiceValue)
        );

        // Auto-move if there's only one valid move
        if (movesToShow.length === 1) {
          setGameMessage(
          ...
          setTimeout(() => {
            console.log("Auto-moving user token:", {
              playerIndex: 0,
              tokenIndex: movesToShow[0],
              diceValue: newDiceValue,
            });
            moveToken(0, movesToShow[0], newDiceValue);
          }, 1000);
        } else {
        ...
        setGameMessage(
          `You rolled ${newDiceValue} but have no valid moves. Bot's turn!`
        );
  ...
  const handleTokenClick = (playerIndex, tokenIndex) => {
    ...
    // Stop token clicking if there's a winner
    if (gameState.winner !== null) {
      console.log("Game has a winner, token clicking blocked");
      return;
    }
    ...
    // Ensure we're moving the user's token (playerIndex 0)
    console.log("Calling moveToken for user token:", {
      playerIndex: 0,
      tokenIndex,
      diceValue,
    });
  ...
  const PaymentModal = () => {
    ...
    return (
      <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl border-4 border-blue-400">
          <div className="text-center">
            ...
            <p className="text-black/90 text-xs mb-6 drop-shadow">
              Searching for opponents in {selectedRoom?.name}
            </p>
  ...
  const WinningScreen = () => {
    ...
    // Play win sound when user wins - only once
    useEffect(() => {
      if (gameState.winner === 0 && !gameProcessed && !hasPlayedSound) {
        if (gameWonAudioRef.current) {
          gameWonAudioRef.current.volume = 0.9;
          gameWonAudioRef.current.currentTime = 0;
          gameWonAudioRef.current.play().catch((err) => {
    ...
    }, [gameState.winner, gameProcessed, hasPlayedSound]);

    // Mark winning screen as shown when winner is determined
    useEffect(() => {
      if (gameState.winner === 0 && !winningScreenShown) {
        setWinningScreenShown(true);
      }
    }, [gameState.winner]);

    // Only show winning screen once - don't render if already shown or game processed
    if (!winningScreenShown || gameProcessed) {
      return null;
    }
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const resultResponse = await fetch(
                        "/api/ludo/process-result",
                        {
                          ...
                          body: JSON.stringify({
                            phone: phone,
                            isWinner: true,
                            prizeAmount: selectedRoom?.prizePool || 0,
                            roomId: selectedRoom?.id || "unknown",
                            roomName: selectedRoom?.name || "Unknown Room",
                          }),
                        }
                      ...
                      if (resultResponse.ok) {
                        // Try to update session as completed (optional - don't fail if session doesn't exist)
                        if (sessionId) {
                          try {
                            await fetch("/api/ludo/sessions/update", {
                              method: "POST",
                        ...
                        window.location.href = "/games/ludo/TwoPlayerLudo";
                      } else {
                        ...
                        if (
                          errorMessage.includes("No pending Ludo bet found")
                        ) {
                          alert(
                            "Game already processed. Redirecting to Ludo..."
                          );
                          ...
                          alert("Failed to process result: " + errorMessage);
                        }
                      }
                    } catch (error) {
  ...
  const LosingScreen = () => {
    ...
    // Mark losing screen as shown when winner is determined
    useEffect(() => {
      if (gameState.winner === 1 && !losingScreenShown) {
        setLosingScreenShown(true);
      }
    }, [gameState.winner]);

    // Only show losing screen once - don't render if already shown or game processed
    if (!losingScreenShown || gameProcessed) {
      return null;
    }
  ...
  // Test Component for Ludo API Testing
  // const TestLudoAPI = () => {
  ...
  //       if (testType === "place-bet") {
  //         // Test placing a bet
  //         const response = await fetch("/api/ludo/place-bet", {
  //           method: "POST",
  //           headers: { "Content-Type": "application/json" },
  //           body: JSON.stringify({
  //             phone: phone,
  ...
  //         if (response.ok) {
  //           setUserWallet(result.newBalance);
  //         }
  //       } else if (testType === "process-win") {
  //         // Test processing a win
  //         const response = await fetch("/api/ludo/process-result", {
  //           method: "POST",
  //           headers: { "Content-Type": "application/json" },
  //           body: JSON.stringify({
  //             phone: phone,
  ...
  //           setUserWallet(result.newBalance);
  //         }
  //       } else if (testType === "process-loss") {
  //         // First place a bet, then process loss
  //         const initialBalance = userWallet;
  //         console.log("Initial balance before loss test:", initialBalance);

  //         const betResponse = await fetch("/api/ludo/place-bet", {
  //           method: "POST",
  //           headers: { "Content-Type": "application/json" },
  //           body: JSON.stringify({
  //             phone: phone,
  //             betAmount: 50,
  //             roomId: "test-loss-room",
  //             roomName: "Test Loss Room",
  //           }),
  //         });
  ...
  //           ...prev,
  //           {
  //             type: "Loss Test - Place Bet",
  //             success: betResponse.ok,
  //             data: betResult,
  //             timestamp: new Date().toLocaleTimeString(),
  //           },
  //         ]);
  ...
  //         if (betResponse.ok) {
  //           setUserWallet(betResult.newBalance);
  //           console.log("Balance after bet placement:", betResult.newBalance);

  //           // Wait a bit, then process loss
  //           await new Promise((resolve) => setTimeout(resolve, 500));

  //           // Now test processing a loss (no additional deduction)
  //           const response = await fetch("/api/ludo/process-result", {
  //             method: "POST",
  //             headers: { "Content-Type": "application/json" },
  //             body: JSON.stringify({
  ...
  //           if (response.ok) {
  //             setUserWallet(result.newBalance);
  //             console.log("Balance after loss processing:", result.newBalance);

  //             // Verify no additional deduction
  //             const expectedBalance = initialBalance - 50; // Only the initial bet amount
  //             const actualBalance = result.newBalance;

  //             if (actualBalance === expectedBalance) {
  //               setTestResults((prev) => [
  //                 ...prev,
  //                 {
  //                   type: "Loss Test - Balance Verification",
  //                   success: true,
  //                   data: {
  //                     expected: expectedBalance,
  //                     actual: actualBalance,
  //                     message:
  //                       "✅ No additional deduction on loss - Balance correct!",
  //                   },
  //                   timestamp: new Date().toLocaleTimeString(),
  //                 },
  //               ]);
  ...
  //                     expected: expectedBalance,
  //                     actual: actualBalance,
  //                     message: "❌ Unexpected balance change on loss!",
  //                   },
  //                   timestamp: new Date().toLocaleTimeString(),
  //                 },
  //               ]);
  ...
  //         // Test the complete flow: bet → win
  //         const initialBalance = userWallet;
  //         console.log("Initial balance before full flow test:", initialBalance);
  ...
  //             phone: phone,
  //             betAmount: 100,
  //             roomId: "full-flow-room",
  //             roomName: "Full Flow Room",
  //           }),
  //         });
  ...
  //           // Wait a bit, then process win
  //           await new Promise((resolve) => setTimeout(resolve, 500));

  //           const winResponse = await fetch("/api/ludo/process-result", {
  //             method: "POST",
  //             headers: { "Content-Type": "application/json" },
  //             body: JSON.stringify({
  //               phone: phone,
  ...
  //             ...prev,
  //             {
  //               type: "Full Flow - Process Win",
  //               success: winResponse.ok,
  //               data: winResult,
  //               timestamp: new Date().toLocaleTimeString(),
  //             },
  //           ]);
  ...
  //           if (winResponse.ok) {
  //             setUserWallet(winResult.newBalance);
  //             console.log(
  //               "Balance after win processing:",
  //               winResult.newBalance
  //             );

  //             // Verify final balance
  //             const expectedBalance = initialBalance - 100 + 200; // Bet deducted + prize added
  //             const actualBalance = winResult.newBalance;

  //             if (actualBalance === expectedBalance) {
  //               setTestResults((prev) => [
  //                 ...prev,
  //                 {
  //                   type: "Full Flow - Final Balance Check",
  //                   success: true,
  //                   data: {
  //                     expected: expectedBalance,
  //                     actual: actualBalance,
  ...
  //                     expected: expectedBalance,
  //                     actual: actualBalance,
  //                     message: "❌ Unexpected final balance!",
  //                   },
  //                   timestamp: new Date().toLocaleTimeString(),
  //                 },
  //               ]);
  ...
  //           }
  //         }
  //       } else if (testType === "test-session") {
  //         // Test session creation and management
  //         const sessionResponse = await fetch("/api/ludo/sessions", {
  //           method: "POST",
  //           headers: { "Content-Type": "application/json" },
  //           body: JSON.stringify({
  //             roomId: "test-session-room",
  //             roomName: "Test Session Room",
  //             playerPhone: phone,
  //             betAmount: 100,
  //             prizePool: 200,
  //           }),
  ...
  //           // Test session update
  //           const updateResponse = await fetch("/api/ludo/sessions/update", {
  //             method: "POST",
  //             headers: { "Content-Type": "application/json" },
  //             body: JSON.stringify({
  ...
  //           ]);
  //         }
  //       } else if (testType === "quick-win") {
  //         // Quick test: Place bet and immediately win
  //         const initialBalance = userWallet;
  //         console.log("Initial balance before quick win test:", initialBalance);
  ...
  //             phone: phone,
  //             betAmount: 100,
  //             roomId: "quick-win-room",
  //             roomName: "Quick Win Room",
  //           }),
  //         });
  ...
  //                 ...prev,
  //                 {
  //                   type: "Quick Win - Balance Verification",
  //                   success: true,
  //                   data: {
  //                     expected: expectedBalance,
  //                     actual: actualBalance,
  //                     message:
  //                       "✅ Win calculation correct - Balance updated properly!",
  //                   },
  //                   timestamp: new Date().toLocaleTimeString(),
  //                 },
  //               ]);
  ...
  //                     expected: expectedBalance,
  //                     actual: actualBalance,
  //                     message: "❌ Unexpected balance after win!",
  //                   },
  //                   timestamp: new Date().toLocaleTimeString(),
  //                 },
  //               ]);
  ...
  //             testType === "place-bet"
  //               ? "Place Bet"
  //               : testType === "process-win"
  //               ? "Process Win"
  //               : testType === "process-loss"
  //               ? "Process Loss"
  //               : testType === "test-session"
  //               ? "Session Test"
  //               : testType === "quick-win"
  //               ? "Quick Win"
  //               : "Unknown Test",
  ...
  //   return (
  //     <motion.div
  //       className="fixed top-4 left-4 bg-black/80 backdrop-blur-sm rounded-lg p-4 max-w-md z-50 border border-white/20"
  //       initial={{ opacity: 0, x: -100 }}
  //       animate={{ opacity: 1, x: 0 }}
  //       transition={{ duration: 0.5 }}
  //     >
  //       <div className="text-white mb-4">
  //         <h3 className="text-lg font-bold mb-2">🧪 Ludo API Test</h3>
  //         <p className="text-sm text-gray-300 mb-3">
  //           Test wallet transactions & game flow
  //         </p>
  //         <p className="text-xs text-yellow-300 mb-2">
  //           💡 Quick Win: Bet→Win instantly | Full Flow: Complete game cycle
  //         </p>

  //         <div className="flex flex-wrap gap-2 mb-4">
  //           <button
  //             onClick={() => runTest("place-bet")}
  //             disabled={isTesting}
  ...
  //             <span className="font-bold">{userWallet}</span>
  //           </div>
  //         </div>
  //       </div>
  ...
  //             <div key={index} className="mb-2 p-2 bg-white/10 rounded text-xs">
        ...
        tokens.push(
          ...
          >
            ...
            >
              ...
              {tokensAtPosition.length > 1 && index === 0 && (
                <>
                  {/* Check if this is a mixed-color stack */}
                  {(() => {
                    ...
                    const isMixedColor = redTokens > 0 && yellowTokens > 0;

                    if (isMixedColor) {
                      // Show mixed-color indicators
                      return (
                        <>
                          {/* Red token count indicator */}
                          {redTokens > 0 && (
                            <motion.div
                              className="absolute md:-top-3 -top-[22px]  md:right-0 right-[8px] text-white rounded-full flex items-center justify-center pointer-events-none z-10000"
                              style={{
                                width:
                                  isMobile && isPortrait
                                    ? "14px"
                          ...
                          {/* Yellow token count indicator */}
                          {yellowTokens > 0 && (
                            <motion.div
                              className="absolute md:-top-0 md:right-0 -top-3 right-[4px] text-black rounded-full flex items-center justify-center pointer-events-none -z-10"
                              style={{
                                width:
                                  isMobile && isPortrait
                                    ? "14px"
                            ...
                            backgroundColor:
                              playerIndex === "0"
                                ...
                                : playerIndex === "1"
                                  ? "#FFDE16"
                                  : "#CF1D1B",
                            color: playerIndex === "1" ? "#000" : "#fff", // Black text for yellow background
                          }}
                          initial={{ scale: 0 }}
                          animate={{ scale: 1 }}
                          transition={{
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen w-screen bg-[url('/ludo/Google.jpeg')] bg-cover bg-center flex flex-col items-center justify-center overflow-hidden">
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-8 max-w-md w-full shadow-2xl border-4 border-blue-400">
          <div className="text-center">
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-8 max-w-md w-full shadow-2xl border-4 border-red-400">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-6">
              Please login to play Ludo. You will be redirected to the login
              page.
            </p>
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 px-6 bg-blue-600 text-white rounded-lg font-bold text-lg hover:bg-blue-700 transition-colors shadow-lg"
              >
                Login Now
              </button>
              <button
      ...
      {showGameList && (
        ...
        >
          ...
          <div className="text-center md:mb-16 mb-8 px-4 md:px-6 lg:px-8">
            <h1 className="text-2xl md:text-6xl font-black tracking-tight mt-4 text-black md:mb-4 mb-2 drop-shadow-[0_2px_8px_rgba(255,255,255,0.2)]">
              <Link href="/dashboard">
                <FaArrowLeft />
              </Link>
              🎲 Two Player Ludo Game Rooms
            </h1>
        ...
        >
          ...
          <div className="flex flex-row gap-2 md:gap-4 lg:gap-6 w-full ">
            <div className="flex p-2 w-full">
              {/* 3 Points around the circle - Bot Timer (now yellow in top-left) */}
              <div
              ...
              <div
                className={`${isMobile && isPortrait
                  ...
                  : isMobile
                    ? "w-20 h-10"
                    : "w-24 h-12"
                  } border-2 rounded-lg ${currentPlayer === 1
                    ? "border-yellow-400 bg-yellow-100"
                    : "border-[#CFBC40] bg-[#A8CAAB]"
                  } p-1`}
              >
                    ...
                    <motion.div
                      className={`${isMobile && isPortrait
                        ...
                        : isMobile
                          ? "w-5 h-5"
                          : "w-6 h-6"
                        } bg-white border border-gray-300 rounded shadow-sm flex items-center justify-center transition-colors relative z-10`}
              ...
              >
                ...
                >
                  {[0, 1, 2, 3].map((i) => (
                    ...
                    >
                      {/* Single colored circle for green tokens (same as red/yellow) */}
                      <div
                        className={`absolute inset-2 rounded-full pointer-events-none`}
                        style={{
                          backgroundColor: "#019F4A",
                          zIndex: 1,
                        }}
                      />
                      <img
              ...
              <motion.div
                className={`w-[40%] h-full ${isMobile && isPortrait
                  ...
                  : isMobile
                    ? "border-[6px]"
                    : "border-[35px]"
                  } border-blue-600 bg-white ${isMobile && isPortrait ? "p-0.5" : isMobile ? "p-1" : "p-4"
                  } relative opacity-50`}
                initial={{ opacity: 0 }}
                animate={{ opacity: 0.5 }}
              ...
              >
                ...
                >
                  {[0, 1, 2, 3].map((i) => (
                    ...
                    >
                      {/* Single colored circle for blue tokens (same as red/yellow) */}
                      <div
                        className={`absolute inset-2 rounded-full pointer-events-none`}
                        style={{
              ...
              <div
                className={`${isMobile && isPortrait
                  ...
                  : isMobile
                    ? "w-20 h-10"
                    : "w-24 h-12"
                  } border-2 rounded-lg ${currentPlayer === 0
